    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      {approvals.length === 0 ? (
        ...
        <div className="space-y-4">
          {approvals.map((item) => {
            const caseNum = item.exceptionCase?.caseNumber || item.run?.exceptionCase?.caseNumber || 'EXC-REF';
            const customer = item.exceptionCase?.customerName || item.run?.exceptionCase?.customerName || 'Enterprise Account';
            const amountVal = item.exceptionCase?.amount || item.run?.exceptionCase?.amount || 0;
            const excType = item.exceptionCase?.exceptionType || 'POLICY_REVIEW';
            ...
            return (
              <div key={item.id} className="rounded-xl border border-zinc-800 bg-[#111114] p-5 shadow-sm space-y-4">
                ...
                <div className="rounded-lg border border-indigo-500/30 bg-indigo-500/10 p-3.5 flex items-start gap-2.5 text-xs">
                  ...
                  <div>
                    <span className="font-semibold text-indigo-300">Agent Proposed Action:</span>
                    <p className="mt-0.5 text-indigo-200">{item.proposedAction}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-zinc-800 text-xs">
                  ...
                  {item.status === 'PENDING' || item.status === 'Pending' ? (
                    ...
                    <span className={`font-semibold px-2.5 py-1 rounded ${
                      item.status === 'APPROVED' || item.status === 'Approved' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'
                    }`}>
                      {item.status}
                    </span>
                  )}
